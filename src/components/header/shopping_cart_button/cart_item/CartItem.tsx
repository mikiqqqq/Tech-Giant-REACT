    ...
    <div className={style.cart_item} key={orderItem.id}>
      <div className={style.cart_item_body}>
        ...
        <div className={style.info_wrapper}>
          <div className={`${style.cart_item_title} not_mobile u-pb1`}>{product.title}</div>
          <div className={`${style.cart_item_title} not_desktop not_pocket u-pb1`}>{product.title.length > 23 ? product.title.slice(0, 20).concat('...') : product.title}</div>
